...
//    var hasNoticedNetworkAimTagIssue = false
//    var hasNoticedNetworkSpeedIssue = false
//    var lastAimUpdate : Date?
//    let formatter : DateComponentsFormatter =  {
//        let formatterPre = DateComponentsFormatter()
//        formatterPre.allowedUnits = [.second]
//        formatterPre.unitsStyle = .full
//        return formatterPre
//    }()
//
...
//            }
//            //MARK:- Calculate Last Aim Update and calculate delay
//            let now = Date()
//            let difference = formatter.string(from: self.lastAimUpdate!, to: now)!
//            //MARK:- Notice User In Case of Slow Network and Delayed Aim Tag Setting
//            if Int( String( difference.first! ) )! >= 3 && hasNoticedNetworkSpeedIssue == false {
//                let alert = makeAlert(withTitle: "네트워크 문제".localized() , withContents: "네트워크 속도가 느립니다 . 이 경우 에임 태그가 지연되어 설치됩니다.".localized())
//                let action = UIAlertAction(title: "확인".localized(), style: .default, handler: {_ in
//                    alert.removeFromParent()
//                })
//                alert.addAction( action )
//                self.present(alert, animated: true, completion: nil)
//                self.hasNoticedNetworkSpeedIssue = true
//            }
//            self.lastAimUpdate = now
...
//                            let alert = makeAlert(withTitle: "네트워크 오류".localized() , withContents: "네트워크가 연결되어있지 않습니다. 이 경우 에임 태그가 정상적으로 작동하지 않습니다.".localized())
//                            let action = UIAlertAction(title: "확인".localized(), style: .default, handler: {_ in
//                                alert.removeFromParent()
//                            })
//                            alert.addAction( action )
