...
class CurrentLocationViewController: UIViewController, CLLocationManagerDelegate {
            ...
            if currentLocationState == CurrentLocationState.completeLoc && performingGeocoding == false{
                ...
                geoCoder.reverseGeocodeLocation( onceConfirmedLocation ){ placemarks, error in
                    if let placemarks = placemarks {
                        ...
                        self.makeAlert( withTitle: "에러", withContents: "인터넷이 연결되어 있지 않거나, 조회가 되지 않는 주소입니다." )
    ...
    @IBAction func getLocation(_ sender: Any) {
        ...
        if authStatus == .denied || authStatus == .restricted {
            ...
            makeAlert(withTitle: "위치 서비스 미허용", withContents:
                        "이 앱을 사용하기 위해서는 위치 서비스 허용이 필요합니다. 기기 전체에 대한 위치 서비스가 허용되어 있는지 확인하고, 이 앱에 대한 위치 서비스를 허용해주세요.")
            return
        }
        // 이미 위치가 잡혔고, 중간에 중단한다면.
        if updatingLocation {
            print("위치 업데이트 중 유저가 업데이트 중단")
    ...
    func locationManager(_ manager: CLLocationManager, didFailWithError error: Error) {
        ...
        //
            ...
            currentLocationState = .locSerDeniedDetected
            makeAlert(withTitle: "위치 서비스 비활성화", withContents: "기기의 위치 서비스 또는 이 앱의 위치 서비스 활성화 여부를 확인해주세요.")
            return
        }
    }
    ...
    @objc func didTimeOut(){
        if location == nil {
            ...
            makeAlert(withTitle: "위치 정보 가져오기 실패", withContents: "gps, 셀룰러, 와이파이 중 하나를 활성화 해주세요.")
            print("시스템이 위치를 가져오는 데 실패.")
        }
    }
        // 3).
    ...
    func stopLocationMananger() {
        ...
        //
        if location != nil {
            print( "\(location!.coordinate.latitude) \(location!.coordinate.longitude) lastest coordinate fetch." )
        }
        //
    ...
    func configureLabel() {
        ...
        if [ CurrentLocationState.unknownTill6 , CurrentLocationState.updatingLoc ].contains(currentLocationState){
            ...
            switch currentLocationState {
                case .unknownTill6:
                    indicatorLabel?.text = "나의 위치 잡는 중"
                case .updatingLoc:
                    indicatorLabel?.text = "위치 정확도 향상중"
                default:
                    break
            }
            //
